    ...
    ***************************************************************************** */
    ...
    function () {
      function RouterView(element, container, viewSlot, router, viewLocator, compositionTransaction, compositionEngine) {
        ...
        (this || _global).compositionTransaction = compositionTransaction;
        (this || _global).compositionEngine = compositionEngine; // add this <router-view/> to router view ports lookup based on name attribute
        // when this router is the root router-view
        // also trigger AppRouter registerViewPort extra flow

        (this || _global).router.registerViewPort(this || _global, (this || _global).element.getAttribute('name')); // Each <router-view/> process its instruction as a composition transaction
        ...
        if (!('initialComposition' in compositionTransaction)) {
          ...
          (this || _global).compositionTransactionNotifier = compositionTransaction.enlist();
        }
      }
      /**@internal */
      ...
      RouterView.prototype.bind = function (bindingContext, overrideContext) {
        ...
        (this || _global).container.viewModel = bindingContext;
      ...
      RouterView.prototype.process = function ($viewPortInstruction, waitToSwap) {
        var _this = this || _global; // have strong typings without exposing it in public typings, this is to ensure maximum backward compat
        ...
        childContainer.get(RouterViewLocator)._notify(this || _global); // layoutInstruction is our layout viewModel


        var layoutInstruction = {
          viewModel: viewPortConfig.layoutViewModel || config.layoutViewModel || (this || _global).layoutViewModel,
          view: viewPortConfig.layoutView || config.layoutView || (this || _global).layoutView,
          model: viewPortConfig.layoutModel || config.layoutModel || (this || _global).layoutModel,
          router: viewPortInstruction.component.router,
          childContainer: childContainer,
          viewSlot: (this || _global).viewSlot
        ...
        var viewStrategy = (this || _global).viewLocator.getViewStrategy(component.view || viewModel);
        ...
        return metadata.load(childContainer, viewModelResource.value, null, viewStrategy, true) // for custom element, viewFactory typing is always ViewFactory
      ...
      RouterView.prototype.swap = function ($viewPortInstruction) {
        ...
        var previousView = (this || _global).view; // Final step of swapping a <router-view/> ViewPortComponent
        ...
        var ready = function (owningView_or_layoutView) {
          ...
          var transactionOwnerShipToken = _this.compositionTransactionOwnershipToken; // if this router-view is the root <router-view/> of a normal startup via aurelia.setRoot
          ...
          if (transactionOwnerShipToken) {
            return transactionOwnerShipToken.waitForCompositionComplete().then(function () {
        ...
        if (layoutInstruction) {
          ...
          return (this || _global).compositionEngine // first create controller from layoutInstruction
          // and treat it as CompositionContext
          // then emulate slot projection with ViewPortComponent view
          .createController(layoutInstruction).then(function (layoutController) {
            var layoutView = layoutController.view;
            aureliaTemplating.ShadowDOM.distributeView(viewPortController.view, layoutController.slots || layoutView.slots); // when there is a layout
        ...
        (this || _global).view = viewPortController.view;
        return ready((this || _global).owningView);
      };
      /**
      ...
      RouterView.prototype._notify = function () {
        var notifier = (this || _global).compositionTransactionNotifier;

        if (notifier) {
          notifier.done();
          (this || _global).compositionTransactionNotifier = null;
        }
      };
      /**
    ...
    */
    ...
    function () {
      ...
      */
      function RouterViewLocator() {
        ...
        (this || _global).promise = new Promise(function (resolve) {
    ...
    function (_super) {
      ...
      TemplatingRouteLoader.prototype.loadRoute = function (router, config, _navInstruction) {
        ...
        return this.resolveViewModel(router, config).then(function (viewModel) {
          return _this.compositionEngine.ensureViewModel({
            viewModel: viewModel,
    ...
    function () {
      ...
      RouteHref.prototype.attributeChanged = function (value, previous) {
        if (previous) {
          (this || _global).element.removeAttribute(previous);
        }
      ...
      RouteHref.prototype.processChange = function () {
        ...
        return (this || _global).router.ensureConfigured().then(function () {
          if (!_this.isActive) {
      ...
      RouteHref.$resource = {
        ...
        bindables: [{
          ...
          name: 'params',
          changeHandler: 'processChange'
        }, 'attribute'] // type definition of Aurelia templating is wrong
    ...
    function configure(config) {
      config.singleton(aureliaRouter.RouteLoader, TemplatingRouteLoader).singleton(aureliaRouter.Router, aureliaRouter.AppRouter).globalResources(RouterView, RouteHref);
